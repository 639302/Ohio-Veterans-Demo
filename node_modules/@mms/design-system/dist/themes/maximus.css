...
[data-theme="maximus"] {
  ...
  /* Avenir Next ships with this package — consumers import
     '@mms/design-system/fonts.css' to load it. This sheet names the family
     only, so the fallback still matters wherever that import is absent.
     Arial rather than system-ui: system-ui resolves to a different face on
     every OS (San Francisco, Segoe UI, Roboto), which makes the brand look
     different per platform. Arial is present everywhere and metric-aliased on
     Linux, so type is at least consistent when Avenir Next is not loaded. */
  --font-family-heading: 'Avenir Next', Arial, Helvetica, sans-serif;
  --font-family-body: 'Avenir Next', Arial, Helvetica, sans-serif;
  ...
  --color-surface-solid: var(--white); /** Pure white surface fill — opt-in for components that need a flat solid background instead of the tinted --color-surface-default (e.g. mms-card surface="solid") */
