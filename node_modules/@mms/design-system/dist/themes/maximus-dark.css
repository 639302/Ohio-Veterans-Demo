...
[data-theme="maximus"][data-mode="dark"] {
  ...
  /* Fallback stack matches maximus.css — see the note there. Arial rather than
     system-ui so the brand renders consistently across operating systems when
     the licensed Avenir Next is not installed. */
  --font-family-heading: 'Avenir Next LT Pro', Arial, Helvetica, sans-serif;
  --font-family-body: 'Avenir Next LT Pro', Arial, Helvetica, sans-serif;
  ...
  --color-action-text: var(--color-primary-12); /** Brand text for secondary/ghost buttons — palette step 12 (Text). Step 9 is Solid (anchor): a fill you place text *on*, not a text colour. */
  ...
  --color-text-on-muted: #1a1a1a; /** Text on light-tint utility fills (toast, banner). Fixed across all themes/modes. */
  ...
  --color-surface-solid: var(--near-black); /** Pure near-black surface fill — opt-in for components that need a flat solid background instead of the tinted --color-surface-default (e.g. mms-card surface="solid") */
  --color-surface-sunken: var(--color-neutral-3); /** Inset element fills — progress-bar track, code blocks. Palette step 3 (Element): a fill *inside* a container, not a surface a control is placed on. */
...
/* ─────────────────────────────────────────────────────────────────────────
   Generated by scripts/build-package.mjs — do not edit dist/ directly.
   Applies the dark values above when the browser or OS requests dark mode
   and the page has not chosen a mode itself. Setting data-mode="light" or
   data-mode="dark" opts out of this entirely.
   ───────────────────────────────────────────────────────────────────────── */
@media (prefers-color-scheme: dark) {
  [data-theme="maximus"]:not([data-mode]) {
